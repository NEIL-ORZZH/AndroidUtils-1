    ...
    public static final class Builder {
        ...
        public Toast build() {
            ...
            if (customeView != null) {
                ...
                toast = Toast.makeText(context, text, isLengthShort ? Toast.LENGTH_SHORT
                        : Toast.LENGTH_LONG);

                if (textColor != -1 || textColorResId != -1 || textSize != -1) {
                    TextView msg = (TextView) toast.getView().findViewById(android.R.id.message);
                    msg.setTextColor(textColor != -1 ? textColor : context.getResources().getColor(
                            textColorResId));
                    if (textSize != -1) {
                        msg.setTextSize(textSize);
                    } else if (textSizeResId != -1) {
                        msg.setTextSize(TypedValue.COMPLEX_UNIT_PX, context.getResources()
                                .getDimension(textSizeResId));
                    }
                }
                ...
                if (paddingLeft != Integer.MIN_VALUE || paddingTop != Integer.MIN_VALUE
                        || paddingRight != Integer.MIN_VALUE || paddingBottom != Integer.MIN_VALUE) {
